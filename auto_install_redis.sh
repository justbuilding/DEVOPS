...
#编译安装
cd /usr/local/src
rm redis-5.0.3.tar.gz redis-5.0.3 -rf
wget http://download.redis.io/releases/redis-5.0.3.tar.gz
#wget http://www.51centos.com/redis-5.0.3.tar.gz
tar -zvxf redis-5.0.3.tar.gz
mv /usr/local/src/redis-5.0.3  /usr/local/src/redis
cd redis
make -j4
...
########写入redis.conf配置文件,根据具体redis配置需求进行配置########
(cat << EOF
